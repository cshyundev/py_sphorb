...
namespace cv
{
	...
	static void initSORB()
	{
		...
		for (int i=0; i<levels; i++)
		{
			...
			bool r = read_pfm(fileName, geoinfo);
			geoinfos.push_back(geoinfo);

			// look up table for fast image conversion from spherical image to geodesic grid
			std::vector<float*> partInfos;
			for (int j=0; j<5; j++)
			{
			...
			Mat mask = imread(fileName, IMREAD_GRAYSCALE);  // OpenCV 4.x에서 상수 변경
    ...
    float numerator = x1 * x2 + y1 * y2 + z1 * z2;
    float denominator = sqrt((x1 * x1 + y1 * y1 + z1 * z1) * (x2 * x2 + y2 * y2 + z2 * z2));
    float tmp = numerator / denominator;

    // Clamp the value of tmp to the range [-1, 1] to avoid NaNs from acos
    if (tmp > 1.0f)
        tmp = 1.0f;
    if (tmp < -1.0f)
        tmp = -1.0f;
	...
	// Calculates the angle based on the moment gradient at a given point
	static float IC_Angle(const cv::Mat& img, const int half_k, cv::Point2f pt, const float* geoinfo)
	{
    float m_01 = 0, m_10 = 0;
    float sqrt3_half = sqrt(3.0f) * 0.5f; // Precompute sqrt(3)/2 for efficiency

    // Get pointer to the center pixel
    const uchar* center = img.ptr<uchar>(cvRound(pt.y)) + cvRound(pt.x);
    ...
    for (int y = -half_k; y <= half_k; ++y)
    {
        ...
        {
            // Calculate Euclidean coordinates based on a hexagonal-like grid
            float euclidX = x + y * 0.5f;
            float euclidY = sqrt3_half * y;

            // Calculate moments m_10 and m_01
            m_10 += euclidX * center[x + y * img.step1()];
            m_01 += euclidY * center[x + y * img.step1()];
        }
    }
	...
	// Computes the orientation for each keypoint in the given image
	static void computeOrientation(const cv::Mat& image, std::vector<cv::KeyPoint>& kps, int halfPatchSize, const float* geoinfo)
	{
    // Loop over all keypoints and calculate the orientation
    for (size_t i = 0; i < kps.size(); ++i)
    {
        // Compute the angle for each keypoint and assign it to the keypoint's angle property
        kps[i].angle = IC_Angle(image, halfPatchSize, kps[i].pt, geoinfo);
    }
	}

	static void computeOrbDescriptor(const cv::KeyPoint& kpt, const cv::Mat& img, const cv::Point* pattern, uchar* desc, int dsize)
	{
    ...
    // Get step size (bytes per row)
    int step = (int)img.step;

    // Get pointer to the center pixel of the keypoint
    const uchar* center = &img.at<uchar>(cvRound(kpt.pt.y), cvRound(kpt.pt.x));

    // Helper function to get the pixel value based on the pattern and angle
    auto get_value = [&](int idx) -> int {
        int y = cvRound(pattern[idx].y * a + pattern[idx].x * c);
        int x = cvRound(pattern[idx].x * b - pattern[idx].y * c);
        return center[y * step + x];
    };
	...
	// Computes ORB descriptors for given keypoints in an image
	static void computeDescriptors(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints, 
                               ...
                               int dsize, int WTA_K)
	{
    // Initialize descriptor matrix with zeros (each descriptor is of size dsize)
    descriptors = cv::Mat::zeros((int)keypoints.size(), dsize, CV_8UC1);

    // Iterate through each keypoint and compute its ORB descriptor
    for (size_t i = 0; i < keypoints.size(); ++i)
    {
        // Compute the ORB descriptor for the current keypoint and store it in the descriptor matrix
        computeOrbDescriptor(keypoints[i], image, pattern.data(), descriptors.ptr((int)i), dsize);
    }
	}

	// Maps the keypoints of each level of the five-part storage grid to the original spherical image
	static void mappingKeypoint(const cv::Mat& img, std::vector<cv::KeyPoint>& kps, int edge, const float* geoinfo, int level)
	{
    // Ensure cells array is defined elsewhere and accessible
    // int cells[];  // cells array assumed to be defined externally

    // Calculate scale factor based on the current level
    float scale = static_cast<float>(cells[0]) / static_cast<float>(cells[level]);
    int pWidth = cells[level] * 2 + 1;

    // Precompute cosine and sine values for five angles
    float pcos[5] = {
        cos(0.0),
        cos(2 * CV_PI / 5),
        cos(4 * CV_PI / 5),
    ...
    // Constant factor for spherical to panoramic coordinate conversion
    float c = CV_PI / img.rows;

    // Iterate over each keypoint to map its position to the spherical image
    for (size_t i = 0; i < kps.size(); ++i)
    {
        // Keypoint coordinates in the grid
        // int _x = static_cast<int>(kps[i].pt.x);
        // int _y = static_cast<int>(kps[i].pt.y);

        // Adjust coordinates based on edge
        int x = static_cast<int>(kps[i].pt.x - edge + 1);
        int y = static_cast<int>(kps[i].pt.y - edge);

        // Fetch 3D spherical coordinates from the geoinfo array
        float sX3D = geoinfo[(x + y * pWidth) * 3];
        float sY3D = geoinfo[(x + y * pWidth) * 3 + 1];
        float sZ3D = geoinfo[(x + y * pWidth) * 3 + 2];

        // Apply rotation based on the keypoint's class_id
        float dX3D = pcos[kps[i].class_id] * sX3D - psin[kps[i].class_id] * sY3D;
        float dY3D = pcos[kps[i].class_id] * sY3D + psin[kps[i].class_id] * sX3D;
        float dZ3D = sZ3D;

        // Convert from 3D spherical to 2D panoramic coordinates
        float theta = acos(dZ3D);  // Angle between Z-axis and 3D point
        float phi = atan2(dY3D, dX3D) + CV_PI;  // Angle between X-axis and the projected point

        // Map to panoramic image coordinates
        float panoX = phi / c;
        float panoY = theta / c;

        // Update keypoint properties
        kps[i].size = 31.0f * scale;  // Adjust keypoint size based on the level scale
        kps[i].pt.x = panoX * scale;  // Scaled panoramic X-coordinate
        kps[i].pt.y = panoY * scale;  // Scaled panoramic Y-coordinate
        kps[i].class_id = -1;         // Reset class_id after mapping
        kps[i].octave = level;        // Set the level as the octave for the keypoint
    }
	}
	...
	void SPHORB::operator()(InputArray _image, InputArray _mask, std::vector<cv::KeyPoint>& _keypoints,
	                        OutputArray _descriptors, bool useProvidedKeypoints) const
	{
	    ...
	    // Compute how many features should be detected at each scale space level
	    std::vector<int> nfeaturesPerLevel(nlevels);
	    float factor = 1.0f / std::pow(2.0, 1.0 / 3.0);
	    float ndesiredFeaturesPerScale = nfeatures * (1 - factor) / (1 - std::pow(factor, nlevels));
	    ...
	    const cv::Point* pattern0 = reinterpret_cast<const cv::Point*>(bit_pattern);
	    std::copy(pattern0, pattern0 + 512, std::back_inserter(pattern));
	
	    // Detect and describe features at each level
	    for (int l = 0; l < nlevels; ++l)
	    {
	        // Resize spherical image
	        cv::Size sz(cells[l] * 5, cells[l] * 5 / 2);
	        Mat image(sz, temp.type());
	        resize(temp, image, sz, 0, 0, cv::INTER_AREA);
	
	        // Split the spherical image into five parts
	        Mat subImg[5];
	        for (int i = 0; i < 5; ++i)
	        {
	            subImg[i].create(cv::Size(2 * cells[l] + 1, cells[l] + 1), image.type());
	        ...
	        for (int i = 0; i < 5; ++i)
	        {
	            ...
	            // Detect keypoints and apply non-max suppression
	        ...
	        // Limit the number of features to the desired number per level
	        if (levelKeyPoints.size() > nfeaturesPerLevel[l])
	            cv::KeyPointsFilter::retainBest(levelKeyPoints, nfeaturesPerLevel[l]);
	
	        // Compute orientation for keypoints
	        for (size_t i = 0; i < levelKeyPoints.size(); ++i)
	        {
	            levelKeyPoints[i].angle = IC_Angle(subImg[levelKeyPoints[i].class_id], SPHORB_EDGE,
	                                               levelKeyPoints[i].pt, geoinfos[l]);
	        }
	        ...
	        // Compute descriptors
	        Mat tDesc = Mat::zeros((int)levelKeyPoints.size(), kBytes, CV_8UC1);
	        for (size_t i = 0; i < levelKeyPoints.size(); ++i)
	        {
	            computeOrbDescriptor(levelKeyPoints[i], subImg[levelKeyPoints[i].class_id],
	                                 pattern.data(), tDesc.ptr((int)i), kBytes);
	        }
	
	        descriptors.push_back(tDesc);
	
	        // Map keypoints back to original spherical image
	...
	void SPHORB::detectImpl(const Mat& image, std::vector<cv::KeyPoint>& keypoints, const Mat& mask) const
	{
	    (*this)(image, mask, keypoints, noArray(), false);
	}
	
	void SPHORB::computeImpl(const Mat& image, std::vector<cv::KeyPoint>& keypoints, Mat& descriptors) const
	{
